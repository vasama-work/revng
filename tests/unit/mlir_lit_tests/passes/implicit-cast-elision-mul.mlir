...
module attributes {clift.module} {
  clift.func @f<!f>(%arg0 : !uint8_t) -> !void {
    ...
    clift.expr {
      ...
      // CHECK-DAG: %[[INNER_MUL:[0-9]+]] = clift.mul %[[INNER_LHS]], %[[INNER_RHS]] : !int32_t

      // CHECK-DAG: %[[OUTER_RHS:[0-9]+]] = clift.cast<extend> %arg0 {clift.implicit} : !uint8_t -> !int32_t
      // CHECK-DAG: %[[OUTER_MUL:[0-9]+]] = clift.mul %[[INNER_MUL]], %[[OUTER_RHS]] : !int32_t

      // CHECK-DAG: %[[ASSIGN_RHS:[0-9]+]] = clift.cast<truncate> %[[OUTER_MUL]] {clift.implicit} : !int32_t -> !uint8_t
      // CHECK: %[[ASSIGN:[0-9]+]] = clift.assign %arg0, %[[ASSIGN_RHS]] : !uint8_t
      // CHECK: clift.yield %[[ASSIGN]] : !uint8_t

      %0 = clift.mul %arg0, %arg0 : !uint8_t
      %1 = clift.mul %0, %arg0 : !uint8_t
