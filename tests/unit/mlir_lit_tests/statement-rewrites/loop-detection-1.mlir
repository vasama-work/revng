...
module attributes {clift.module} {
  ...
  clift.func @f<!f>(%arg0 : !int32_t, %arg1 : !int32_t) {
    ...
    // CHECK: clift.while
    // COM: CHECK: break [[OUTER_BREAK:%[0-9]+]]
    // COM: CHECK: continue [[OUTER_CONTINUE:%[0-9]+]]
    // CHECK-SAME: cond {
      // CHECK: [[COND:%[0-9]+]] = clift.imm 1 : !int32_t
      // CHECK: clift.yield [[COND]] : !int32_t
    // CHECK: } body {
    ...
    // CHECK: clift.while
    // COM: CHECK: break [[INNER_BREAK:%[0-9]+]]
    // COM: CHECK: continue [[INNER_CONTINUE:%[0-9]+]]
    // CHECK: cond {
      // CHECK: [[COND:%[0-9]+]] = clift.imm 1 : !int32_t
      // CHECK: clift.yield [[COND]] : !int32_t
    // CHECK: } body {
    ...
    // CHECK: } then {
      ...
      // CHECK: clift.expr {
      clift.expr {
        // CHECK: [[IMM:%[0-9]+]] = clift.imm 30 : !int32_t
      ...
      // CHECK: } then {
        ...
        // CHECK: clift.expr {
        clift.expr {
          // CHECK: [[IMM:%[0-9]+]] = clift.imm 40 : !int32_t
      ...
      // CHECK: } else {
        ...
        // COM: CHECK: clift.continue_to [[OUTER_CONTINUE]]
        clift.goto %outer_loop
      // CHECK: }
      }
    // CHECK: } else {
    } else {
      // COM: CHECK: clift.continue_to [[INNER_CONTINUE]]
      clift.goto %inner_loop
    // CHECK: }
    }

    // COM: CHECK: clift.break_to [[INNER_BREAK]]
    // COM: inner loop closer
    // CHECK: }

    // COM: CHECK: clift.break_to [[OUTER_BREAK]]
