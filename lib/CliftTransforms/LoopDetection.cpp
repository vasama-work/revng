...
static bool isBackwardGoto(GotoOp Goto, AssignLabelOp Label) {
  ...
  return isOrderedBefore(Label->getIterator(),
...
static void createLoop(FunctionOp Function,
  ...
  // If any loop-gotos still exist, they are replaced by continue-to operations
  // targeting a newly created continue label.
  if (not Gotos.empty()) {
