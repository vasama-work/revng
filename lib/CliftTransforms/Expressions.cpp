...
static bool readTypePunnedConstraint(mlir::Value Result, mlir::Value Ptr) {
  auto PtrType = mlir::dyn_cast<PointerType>(Ptr.getType());
  if (not PtrType)
    return false;

  auto SrcType = PtrType.getPointeeType();
  auto DstType = mlir::cast<clift::ValueType>(Result.getType());

  if (SrcType == DstType or SrcType.getByteSize() != DstType.getByteSize())
    return false;

  return not isValueCategorySignificant(Result);
}
...
struct BeautifyExpressionsPass
  : impl::CliftBeautifyExpressionsBase<BeautifyExpressionsPass> {
...
PassPtr<FunctionOp> clift::createBeautifyExpressionsPass() {
