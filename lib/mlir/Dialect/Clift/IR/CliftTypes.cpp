...
static bool getDefinedTypeAliasImpl(DefinedType Type,
  ...
  if (not Type.getName().empty()) {
    ...
    OS << clift::sanitizeIdentifier(Type.getHandle());
  } else {
    OS << Kind;
  }
...
static mlir::ParseResult parseDefinedTypeName(mlir::AsmParser &Parser,
                                              std::string &Name) {
  if (Parser.parseOptionalKeyword("as").succeeded()) {
    if (Parser.parseString(&Name).failed())
      return mlir::failure();
...
static void printDefinedTypeName(mlir::AsmPrinter &Printer,
                                 llvm::StringRef Name) {
  if (not Name.empty()) {
    Printer << " as ";
...
mlir::Type EnumType::parse(mlir::AsmParser &Parser) {
  ...
  std::string Name;
  if (parseDefinedTypeName(Parser, Name).failed())
    return {};
...
void EnumType::print(mlir::AsmPrinter &Printer) const {
  ...
  printDefinedTypeName(Printer, getName());
  ...
  for (auto [I, E] : llvm::enumerate(getFields())) {
    Printer << (I == 0 ? '\n' : ',') << "  " << E.getRawValue();
    printDefinedTypeName(Printer, E.getName());
    Printer << '\n';
  }
  Printer << "}>";
}
...
static void printClassType(TypeT Type, mlir::AsmPrinter &Printer) {
  ...
  Printer << "<";
  printString(Printer, Type.getHandle());
  printDefinedTypeName(Printer, Type.getName());
