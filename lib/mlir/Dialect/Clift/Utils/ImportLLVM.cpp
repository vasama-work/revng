...
public:
  static void import(mlir::ModuleOp ModuleOp,
    ...
    LLVMCodeImporter(ModuleOp.getContext(), Model)
      .importModule(ModuleOp, Module);
  }
...
clift::importLLVM(mlir::MLIRContext *Context,
                  const model::Binary &Model,
                  const llvm::Module *Module) {
  mlir::Location Loc = mlir::FileLineColLoc::get(Context,
                                                 Module->getSourceFileName(),
                                                 0,
                                                 0);

  auto Op = createOperation<mlir::ModuleOp>(Context, Loc);
