...
class LLVMCodeImporter final : public LLVMToCliftImporter {
public:
  ...
  // properties.
  explicit LLVMCodeImporter(mlir::ModuleOp Module, const model::Binary &Model) :
...
std::unique_ptr<LLVMToCliftImporter>
LLVMToCliftImporter::make(mlir::ModuleOp Module, const model::Binary &Model) {
  return std::make_unique<LLVMCodeImporter>(Module, Model);
}
