...
private:
  ...
  mlir::LogicalResult visitFunctionOp(clift::FunctionOp Op) {
    ...
    if (not IsIsolated and not Op.isExternal())
      return getCurrentOp()->emitError() << "Clift ModuleOp contains "
                                            "non-isolated with a definition: '"
                                         << Op.getHandle() << "'";
